...
[🤗 Weights & model card on Hugging Face](https://huggingface.co/AbteeXAILab/lumynax-ocr-trocr-large-handwritten) · [abteex.com](https://abteex.com) · [lumynax.com](https://lumynax.com) · [Monorepo](https://github.com/Aimaghsoodi/lumynax-release)

---

This GitHub repo mirrors the **runtime scaffolding** (quickstart, requirements, Modelfile, Space, docs). The model **weights** are published on Hugging Face at `AbteeXAILab/lumynax-ocr-trocr-large-handwritten` — they exceed GitHub's per-file limit.

```bash
hf download AbteeXAILab/lumynax-ocr-trocr-large-handwritten --local-dir lumynax-ocr-trocr-large-handwritten
cd lumynax-ocr-trocr-large-handwritten && pip install -r requirements.txt && python quickstart.py --interactive
# Or:  pip install lumynax && lumynax run lumynax-ocr-trocr-large-handwritten -i
```
